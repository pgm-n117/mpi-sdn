...
@Component(immediate = true,
           service = {MpiControllerFlowB.class},
           property = {
               "someProperty=Some Default String Value"}
           )
public class MpiControllerFlowB implements MpiControllerFlowBInterface {
    ...
    private class mpiControllerProcessor implements PacketProcessor {
                    ...
                    //UDP TREATMENT
                        ...
                        if (dstIpAddress.equals(MPI_CONTROLLER_IP) & (dstIpPort == 7777)) { //ip address of controller or special ip to identify
                            ...
                            if (Integer.reverseBytes(PeerAddr[2]) == 0) {
                                ...
                                try {
                                synchronized (MPIEndpointsMutex){
                                    //Install Flowrules for MPI endpoints, from each existing endpoint to the new one
                                    Host MPIHost = hostService.getHostsByIp(peerIpAddress).iterator().next();
                                    MPIEndpoints.forEach((host, ports) -> {
                                        if(!host.getIp4Address().equals(peerIpAddress)){
                                            Host actualhost = hostService.getHostsByIp(host.getIp4Address()).iterator().next();
                                            //Set path from existing endpoint to new one
                                            setMPIPath(context, actualhost, MPIHost, IPv4.PROTOCOL_TCP, host.getIp4Address(), peerIpAddress, 0, peerPort);

                                            log.info("*********NEW MPI BTL PATH*********" + host + " -> " + peerIpAddress + ":" + peerPort);

                                            ports.forEach((port, portNumber) -> {
                                                //Set path from new endpoint to existing ones. Once it is done for the first time for each MPIHost, it is not necessary to do it again
                                                //As both hosts are already connected
                                                if(MPIEndpoints.get(peerIpAddress).size() <= 1){ //if it is the first time this host has been added
                                                    setMPIPath(context, MPIHost, actualhost, IPv4.PROTOCOL_TCP, peerIpAddress, host.getIp4Address(), 0, port);
                                                }
                                                log.info("*********NEW MPI BTL PATH*********" + peerIpAddress + " -> " + host + ":" + port);
                                            });
        ...
        private void setMPIPath(PacketContext context, Host sourceHost, Host dstHost, byte protocol, Ip4Address srcIp, Ip4Address dstIpAddress, int srcIpPort, int dstIpPort) {
            ...
            if(!paths.isEmpty()){
                path = paths.iterator().next(); //The first path SHOULD be the best one
                log.info("SELECTED MPI PATH: "+path.toString());
            }
            //Path reversePath = selectMPIPaths(reversePaths, OutputDevicePort, true);


            if (path != null) {
                //log.info("FOUND PATHS FOR HOSTS: "+srcIp.toString()+" - "+dstIpAddress.toString() + " PATH: "+path.links().toString());
        ...
        private FlowRule installMPIPathFlowRule(ConnectPoint dstConnectionPoint, byte protocol, Ip4Address srcIp,
            ...
            //Protocol should always be TCP when entering MPI paths
            if (dstIpPort != 0 )  selector = reverse ? selector.matchTcpSrc(TpPort.tpPort(dstIpPort)) : selector.matchTcpDst(TpPort.tpPort(dstIpPort));
            if (srcIpPort != 0 )  selector = reverse ? selector.matchTcpDst(TpPort.tpPort(srcIpPort)) : selector.matchTcpSrc(TpPort.tpPort(srcIpPort));
...
//                                log.info("*********RESET MPI LINKS********* :\n " + Arrays.asList(ActiveMPILinks));
    ...
    //TODO: CHECK
    public class FlowWeigher extends DefaultEdgeWeigher<TopologyVertex, TopologyEdge> implements LinkWeigher {
        ...
        */
        ...
        public Weight weight(TopologyEdge edge) {
            ...
            try {
                ...
                synchronized (MPILinksMutex) {
                    usage = ActiveMPILinks.getOrDefault(edge.link(), 0.0);
                }

                return ScalarWeight.toWeight(usage);


            } catch (Exception e){
                log.error("Error on LinkWeigher:"+ ExceptionUtils.getStackTrace(e));
                e.printStackTrace();
            }

            log.error("Error on LinkWeigher: OUT OF TRY CATCH: RETURN POSITIVE_INFINITY");
